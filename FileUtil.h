...
public:
    ...
    static std::shared_ptr<Game> loadGame(std::string fileName);

    // Saves the game's data in the file in the given file name
    static void saveGame(std::string fileName, Game* game);

private:
    //Method to get all the data of the player passed.
    static bool getPlayerData(std::shared_ptr<Player> player, std::fstream& inputFile);

    //Method to input tiles from the file and store in the linked list passed as params.
    static bool giveTilesList(std::string tileList, std::shared_ptr<LinkedList> tilList);

    //Method to check if the current tile is in correct format or not.
    static bool isTileCorrect(std::string tile);

    //Method to check if the player name passed as param is in correct format or not.
    static bool isNameCorrect(std::string name);
